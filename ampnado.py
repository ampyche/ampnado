...
class SetUp():
    ...
    def _get_args(self, a_ppath):
        ...
        subparsers = parser.add_subparsers(
            title="Setup AmpNado", description="AmpNado SubCommands",
            help='this is the help message')
        parser_install = subparsers.add_parser('Install')
        parser_install.add_argument("-s", "--serv-addr", required=True,
            help=SERVER_ADDR_HELP)
        parser_install.add_argument("-m", "--media-path", nargs="*",
            required=True, help=MEDIA_PATH_HELP)
        parser_install.add_argument("-c", "--install_catalog-name",
            default=a_uuid, help=CAT_NAME_HELP)
        parser_install.add_argument("-o", "--offset-size", type=int,
            default=15, help=OFFSET_SIZE_HELP)
        parser_install.add_argument("-u", "--username", default='Admin',
            help=UNAME_HELP)
        parser_install.add_argument("-p", "--password", default='ampnado',
            help=PWORD_HELP)
